...
define('DB_USER', 'root');
define('DB_PASS', ''); // XAMPP默认root用户无密码
...
function log_security_event($event, $user_id = null, $details = null) {
    $db = Database::getInstance()->getConnection();
    $ip = $_SERVER['REMOTE_ADDR'] ?? '127.0.0.1';
    $stmt = $db->prepare("INSERT INTO security_logs (event_type, user_id, ip, details, created_at) VALUES (?, ?, ?, ?, NOW())");
    $details_json = is_array($details) ? json_encode($details) : null;
    $stmt->execute([$event, $user_id, $ip, $details_json]);
}
